...
export function clampWeight(weight: number): 1 | 2 | 3 {
  if (weight <= 1) return 1;
  if (weight <= 2) return 2;
  return 3;
}

export function formatScoringOption(
  value: number,
...
export function auditToCheckResult(
  audit: LighthouseResult["audits"][string],
  weight: 1 | 2 | 3,
): CheckResult | null {
...
export interface LighthouseCategoryExtraction {
  metrics: CheckResult[];
...
export function extractLighthouseCategory(
